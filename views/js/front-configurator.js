...
$('#modalformaticustom').on('hidden.bs.modal', function () {
  jQuery('.tool-panel-container .cancel-button').click()
});


$("#width, #height").keypress(function() {
  console.log("test");
  var price_totale = getPriceCustomFormat();
  $("#price_formato_custom").val(price_totale);
  $("#total-price-custom-format").html(formatMoney(addPriceTax(price_totale), 2, ",", ".") + " €");
...
function addFormatoCustom() {
  ...
  $("#formato-selezionato").html("Scelto: <b>"+larghezza_cm+" x "+altezza_cm+"</b>");
  var price_totale = getPriceCustomFormat();
  $("#price_formato_custom").val(price_totale);
  $("#total-price-custom-format").html(formatMoney(addPriceTax(price_totale), 2, ",", ".") + " €");
...
function addFormato(is_default, is_custom) {
  ...
  setTimeout(function(){
    ...
    if (is_custom) {
      ...
      formato_base = $("#formato-selezionato b").text();
    } else {
      $(".box-formato").each(function(){
        if ( $(this).data("id") == EW_idFormatoSelected) {
          ...
          formato_base = $(this).find("span").html();
        }
      });
    }
    ...
    jQuery('.scroll-container .control-button:first-of-type').click();
    var selectedRatio = aspectRatio(formato_base);
    
    if(is_custom) {
      console.log("now add ratio", selectedRatio);
      var ratiosObject = JSON.parse(ratios);
      ratiosObject.push({ratio: selectedRatio, name: formato_base});
      ratios = JSON.stringify(ratiosObject);
      replaceFormats(formato_base);
    }
    else {
      $( ".aspect-ratio .preview[data-ratio='"+selectedRatio+"']" ).click();
      $( ".apply-button" ).click();
    }
...
function customPosition() {
  ...
  setTimeout(function(){
    jQuery('.scroll-container .control-button:first-of-type').click();
    var formato_base = $('.box-formato.selected .small').text();
    console.log(formato_base);
    var selectedRatio = aspectRatio(formato_base);
    $( ".aspect-ratio .preview[data-ratio='"+selectedRatio+"']" ).click();
  }, 300);
}
...
function replaceFormats(custom_ratio) {
  console.log("REPLACE_FORMATS", ratios);

  var res = custom_ratio.split(" ");
  var x1 = res[0];
  var y1 = res[2];
  var maxWidth = 100;
  var maxHeight = 100;
  var width = 1920;
  var height = 1080;
  var ratioW = x1;  // Width ratio
  var ratioH = y1;  // Height ratio

  // If height ratio is bigger then we need to scale height
  if(width > height) {
    console.log({width: height*ratioW/ratioH, height: height, left: 0, top: 0})
    var newWidth = height*ratioW/ratioH;

    var cropTool = pixie.getTool('crop');
    cropTool.apply({width: newWidth, height: height, left: (width-newWidth)/2, top: 0});
  }
  else if(height > width) {
    var newHeight = height*ratioH/ratioW;

    var cropTool = pixie.getTool('crop');
    cropTool.apply({width: width, height: newHeight, left: 0, top: 0});
  }
}